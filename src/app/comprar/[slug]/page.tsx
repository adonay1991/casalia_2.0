...
export async function generateStaticParams() {
	...
	return slugs
		.filter((s) => s.operationType === "venta")
		.map((s) => ({ slug: s.slug }));
}
...
export default async function PropertyPage({ params }: PropertyPageProps) {
	...
	const whatsappMessage = encodeURIComponent(
		`Hola, me interesa la propiedad: ${property.title}. ¿Podrian darme mas informacion?`,
	);
		...
		<>
			...
			<BreadcrumbSchema
				items={[
					...
					{ name: property.title, url: `${BASE_URL}/comprar/${property.slug}` },
				]}
			/>
						...
						<div className="grid grid-cols-2 sm:grid-cols-4 gap-4 p-4 bg-zinc-50 rounded-lg">
							...
							{property.sqmBuilt && (
								<div className="text-center">
									...
									<div className="font-semibold">{property.sqmBuilt} m²</div>
									<div className="text-xs text-muted-foreground">
										Construidos
									</div>
								</div>
						...
						<div className="sticky top-24 space-y-6">
							...
							<div className="bg-card border border-border rounded-lg p-6 shadow-sm">
								...
								<dl className="space-y-3 text-sm">
									...
									{property.sqmBuilt && (
										<div className="flex justify-between">
											<dt className="text-muted-foreground">m² construidos</dt>
											<dd className="font-medium">{property.sqmBuilt} m²</dd>
										</div>
									)}
									{property.sqmUseful && (
										<div className="flex justify-between">
											<dt className="text-muted-foreground">m² utiles</dt>
											<dd className="font-medium">{property.sqmUseful} m²</dd>
										</div>
									)}
									{property.floor && (
										<div className="flex justify-between">
					...
					<div className="container mx-auto px-4 md:px-6">
						...
						<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
							{similarProperties.map((prop) => (
								<PropertyCard key={prop.id} property={prop} variant="comprar" />
							))}
						</div>
					</div>
				</section>
